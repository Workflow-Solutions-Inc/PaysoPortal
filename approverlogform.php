...
<body>-->
	...
	<div id="leftpanel" class="leftpanel">
		...
		<ul class="subbuttons">
			...
			<li><button onClick="ApproveLC();"><span class="fa fa-thumbs-up"></span> Approve</button></li>
			<li><button onClick="DisapproveLC();"><span class="fa fa-thumbs-down"></span> Disapprove</button></li>
						...
						<div id="container1" class="full">
							<table width="100%" border="0" id="datatbl" class="table table-striped mainpanel-table">
								<thead>	
									...
									<tr class="rowsearch">
									  ...
									  <td><input list="SearchDateFiled" class="search" disabled>
										<?php
											$query = "SELECT distinct date_format(createddatetime, '%Y-%m-%d') as datefiled FROM logcorrection where dataareaid = '$dataareaid' and status = 0";
											$result = $conn->query($query);	
								...
								<tbody id="result">
									...
									$query4 = "SELECT *,TIME_FORMAT(lc.logtime,'%h:%i %p') as logtime,
									case when lc.status = 0 then 'Created'
										when lc.status = 1 then 'Approved' 
										when lc.status = 2 then 'Disapproved' 
										when lc.status = 3 then 'Posted' end as otstatus,
									...
									where lc.dataareaid = '$dataareaid' and lc.workerid = '$lognum'
	...
	<div class="modal-container">
		<div class="modal-content">
			...
			<div id="container" class="modal-content-container">
				<form name="myForm" accept-charset="utf-8" action="approverlogformprocess.php" method="get">
					<div class="row">
		...
		var locWorkerId = "";
  		$(document).ready(function(){
			$('#datatbl tbody tr').click(function(){
				...
				locWorkerId = $("#datatbl tr:eq("+ ($(this).index()+2) +") td:eq(12)").text();
				so = usernum.toString();
		...
		$( ".search" ).on( "keydown", function(event) {
		  if(event.which == 13){
			...
			var data=[];
			 ...
			 $.ajax({
						type: 'GET',
						url: 'approverlogformprocess.php',
