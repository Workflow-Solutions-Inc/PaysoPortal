...
<body>-->
	...
	<div class="header">
		...
		<div class="header-nav">
				<ul>
					<li id="pla" onClick="reply_click(this.id)"><a href="logform.php?list=pla"><span class="fas fa-hourglass"></span>  Pending Log Correction Application</a></li>
					<li id="ala" onClick="reply_click(this.id)"><a href="logapprovedform.php?list=ala"><span class="fas fa fa-check"></span>  Posted Log Correction Application</a></li>
				</ul>
		</div>
	...
	<div id="leftpanel" class="leftpanel">
		...
		<ul class="subbuttons">
			...
			<!--<li><button onClick="Sample();"><span class="fa fa-arrow-circle-left fa-lg"></span> Back</button></li>-->
		</ul>
								...
								<thead>	
									...
									<tr class="rowsearch">
									  <td class="text-center"><span class="fas fa-search fa-xs"></span></td>
										...
										<?php
											$query = "SELECT distinct logid FROM logcorrection where dataareaid = '$dataareaid' and workerid = '$lognum' and status = 0";
									  ...
									  <td><input list="SearchName" class="search" >
										<?php
											$query = "SELECT distinct name FROM logcorrection where dataareaid = '$dataareaid' and workerid = '$lognum' and status = 0";
											$result = $conn->query($query);	
									  ...
									  <td><input list="SearchInvalidDate" class="search">
										<?php
											$query = "SELECT distinct invaliddate FROM logcorrection where dataareaid = '$dataareaid' and workerid = '$lognum' and status = 0";
											$result = $conn->query($query);	
									  ...
									  <td><input list="SearchDetails" class="search" disabled>
										<?php
											$query = "SELECT distinct details FROM logcorrection where dataareaid = '$dataareaid' and workerid = '$lognum' and status = 0";
											$result = $conn->query($query);	
									  ...
									  <td><input list="SearchLogtime" class="search" disabled>
										<?php
											$query = "SELECT distinct logtime FROM logcorrection where dataareaid = '$dataareaid' and workerid = '$lognum' and status = 0";
											$result = $conn->query($query);	
									  ...
									  <td><input list="SearchLogtype" class="search" disabled>
										<?php
											$query = "SELECT distinct logtype FROM logcorrection where dataareaid = '$dataareaid' and workerid = '$lognum' and status = 0";
											$result = $conn->query($query);	
									  ...
									  <td><input list="SearchStatus" class="search" disabled>
										<?php
											$query = "SELECT distinct status FROM logcorrection where dataareaid = '$dataareaid' and workerid = '$lognum' and status = 0";
											$result = $conn->query($query);	
									  ...
									  <td><input list="SearchDateFiled" class="search" disabled>
										<?php
											$query = "SELECT distinct date_format(createddatetime, '%Y-%m-%d') as datefiled FROM logcorrection where dataareaid = '$dataareaid' and workerid = '$lognum' and status = 0";
											$result = $conn->query($query);	
...
<script src="js/ajax.js"></script>
	<script  type="text/javascript">
		...
		var loclogtypenum = "";
  		$(document).ready(function(){
			$('#datatbl tbody tr').click(function(){
				...
				locinvaliddate = $("#datatbl tr:eq("+ ($(this).index()+2) +") td:eq(3)").text();
				locDetails = $("#datatbl tr:eq("+ ($(this).index()+2) +") td:eq(4)").text();
				loclogtime = $("#datatbl tr:eq("+ ($(this).index()+2) +") td:eq(9)").text();
				locStatus = $("#datatbl tr:eq("+ ($(this).index()+2) +") td:eq(6)").text();
				locDateFile = $("#datatbl tr:eq("+ ($(this).index()+2) +") td:eq(7)").text();
				loclogtypenum = $("#datatbl tr:eq("+ ($(this).index()+2) +") td:eq(10)").text();
		...
		function checkExistForm()
		{
			...
			var lcdt = document.getElementById("t3").value;
			myFilled = lcdt.toLowerCase().split(",");

			var d = new Date();
			var x = new Date(document.getElementById("add-lcdate").value.toLowerCase());
