	...
	if($_GET["actmode"]=="userform"){
		...
		$query = "SELECT *,TIME_FORMAT(starttime,'%h:%i %p') as timein,TIME_FORMAT(endtime,'%h:%i %p') as timeout,
									case when status = 0 then 'Created'
										when status = 1 then 'Approved' end as otstatus,
					...
					where dataareaid = '$dataareaid' and (overtimeid like '%$id%') and (name like '%$name%') and (overtimedate like '%$overtimedate%')
					 and status = 0
					order by overtimeid";
		$result = $conn->query($query);
		$rowclass = "rowA";
		...
		while ($row = $result->fetch_assoc())
		{
			...
			<tr class="'.$rowclass.'">
				...
				<td style="width:5%;">'.$row["timein"].'</td>
				<td style="width:5%;">'.$row["timeout"].'</td>
